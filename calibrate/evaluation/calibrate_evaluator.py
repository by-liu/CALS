...
class CalibrateEvaluator(Evaluator):
    def __init__(self, num_classes, num_bins=15) -> None:
    ...
    def mean_score(self, print=False, all_metric=True):
        ...
        ece_criterion = ECELoss(self.num_bins).cuda()
        aece_criterion = AdaptiveECELoss(self.num_bins).cuda()
        cece_criterion = ClasswiseECELoss(self.num_bins).cuda()
